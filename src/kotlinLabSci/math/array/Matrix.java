...
// This is the basic Matrix class of kotlinLab.
...
import static kotlinLabSci.PrintFormatParams.*;
import static kotlinLabSci.math.array.NUMALConvArrays.*;
import kotlinLabSci.math.io.files.ASCIIFile;
import java.io.File;
...
public Matrix(kotlinLabSci.math.array.Vec _a)  {
    d = new double[1][1]; 
...
///////////////////////////////////////////////////////////////////
  ...
  final public Matrix plus( double v2)  {
        return new Matrix(LinearAlgebra.plus( getRef(), v2), false );
    }


    final public Matrix plus( int v2)  {
        return new Matrix(LinearAlgebra.plus( getRef(), (double)v2), false );
    }

    final public Matrix minus( int v2)  {
        return new Matrix(LinearAlgebra.plus( getRef(), -(double)v2), false );
    }
...
final public Matrix times( Matrix that) {
       return this.multiply(that);
}

final public Matrix times( Double that) {
    return new Matrix(LinearAlgebra.times( getRef(), that), false );
    }
...
/*
    ...
    if (siz  >  kotlinLabExec.Interpreter.GlobalValues.mulMultithreadingLimit) 
...
final public static double [] solve(Sparse A, double [] b) {
    return kotlinLabSci.math.array.Sparse.sparseSolve(A, b);
}

// solve with the MTJ Column Compressed Matrix
final public static double [] solve(CCMatrix A, double [] b) {
    return kotlinLabSci.math.array.CCMatrix.BiCGSolve(A, b);
}
...
final public EigResults  eigMTJ() {  
  EigResults eigA = kotlinLabSci.math.array.JILapack.EigMTJ(this.getArray());
  return eigA;
}

final public static EigResults  eigMTJ(Matrix A) {  
  EigResults eigA = kotlinLabSci.math.array.JILapack.EigMTJ(A.getArray());
  return eigA;
}

final public static EigResults  eigMTJ(double [][] A) {  
  EigResults eigA = kotlinLabSci.math.array.JILapack.EigMTJ(A);
  return eigA;
}
...
final public static double [][]  DGELS(double [][] A, double [][] B) {
    return kotlinLabSci.math.array.JILapack.DGELS(A, B);
}

final public static Matrix   DGELS(Matrix A, Matrix B) {
    return new Matrix(kotlinLabSci.math.array.JILapack.DGELS(A.getArray(), B.getArray()));
}
...
final public void  browse() {
    kotlinLabExec.gui.watchMatrix.display(this.getArray());
}

final public void  browse(String matrixName) {
    kotlinLabExec.gui.watchMatrix.display(this.getArray(), matrixName);
}
...
public static String   printArray(double [] va) {
   ...
   for (int k = 0; k < kotlinLabSci.PrintFormatParams.vecDigitsPrecision; k++)
       formatString += "0";
    ...
    digitFormat.setDecimalFormatSymbols(new DecimalFormatSymbols(new Locale("us")));
     ...
     String  moreElems = "";
     if (mxElems > kotlinLabSci.PrintFormatParams.vecMxElemsToDisplay )  {
          // vector has more elements than we can display
         mxElems = kotlinLabSci.PrintFormatParams.vecMxElemsToDisplay;
         moreElems = " .... ";
     }
    int  i=0;
     while (i < mxElems) {
