...
public static boolean initAbbreviationsMap()   {
      ...
      if (abbreviationsMapInited == false) {
         abbreviationsMap =  buildAbbreviationsMap(GlobalValues.workingDir+java.io.File.separatorChar+"Abbreviations.txt");
...
// detects and returns the word at the current caret location
  public static  String  detectAndReplaceWordAtCaret()    {
     int caretPosition = GlobalValues.globalEditorPane.getCaretPosition()-1;
  
    String  txt = GlobalValues.globalEditorPane.getText();  // the whole editor's text
    ...
    while (offset >= 0 && exited==false) {
        ...
        GlobalValues.globalEditorPane.setSelectionStart(offset+1);
      ...
      for ( int i = length - 1 ; i >= 0 ; i-- )
        ...
        GlobalValues.globalEditorPane.setSelectionEnd(offset);
    ...
    if (abbreviationsMap.containsKey(wordAtCursor))  {
        String replacingText =  abbreviationsMap.get(wordAtCursor);
            ...
            GlobalValues.globalEditorPane.replaceSelection( replacingText);
    }
